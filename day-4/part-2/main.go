...
func main() {
	...
	bingoHighestNumOfBingo := 0
	bingoCardHighScore := 0
	bingoLowestNumOfBingo := 0
	bingoCardLowScore := 0
	...
	for bingoNumberIndex := 0; bingoNumberIndex < len(bingoNumbers); bingoNumberIndex++ {
		...
		for _, bingoCard := range bingoCards {
			...
			if _, ok := winningBingoCards[bingoCardID]; bingo(bingoNumbers, bingoCard) && !ok {
				...
				sumOfUnmatchedBingoNumbers := sumOfUnmatchedNumbers(bingoNumbers, bingoCard)
				bingoNumber, _ := strconv.Atoi(bingoNumbers[len(bingoNumbers)-1])
				finalScore := sumOfUnmatchedBingoNumbers * bingoNumber

				if len(bingoNumbers) > bingoHighestNumOfBingo {
					bingoHighestNumOfBingo = len(bingoNumbers)
					bingoCardHighScore = finalScore
				}

				if len(bingoNumbers) < bingoLowestNumOfBingo || bingoLowestNumOfBingo == 0 {
					bingoLowestNumOfBingo = len(bingoNumbers)
					bingoCardLowScore = finalScore
				}
			}
		}

	}
	fmt.Println("bingoLowestNumOfBingo :: ", bingoLowestNumOfBingo)
	fmt.Println("bingoCardLowScore :: ", bingoCardLowScore)

	fmt.Println("bingoHighestNumOfBingo :: ", bingoHighestNumOfBingo)
	fmt.Println("bingoCardHighScore :: ", bingoCardHighScore)
...
func bingo(bingoNumbers []string, bingoCard []string) bool {
	...
	verticalBingoCard := convertHorizontalCardToVerital(bingoCard)
	for _, bingoCardNumbers := range verticalBingoCard {
		bingo := true
		for _, bingoCardNumber := range strings.Fields(bingoCardNumbers) {
			if !stringInSlice(bingoCardNumber, bingoNumbers) {
				bingo = false
...
func convertHorizontalCardToVerital(bingoCard []string) []string {
	verticleBingoCardMap := map[int]string{}
	verticleBingoCard := []string{}
	/*
		[
			"3 15  0  2 22",
			"9 18 13 17  5",
	...
	*/
	for _, row := range bingoCard { // row = "3 15  0  2 22",
		for rowNumberIndex, rowNumber := range strings.Fields(row) { // row number = 3
			verticleBingoCardMap[rowNumberIndex] += rowNumber + " "
		}
	}

	// convert map to []string
	for _, value := range verticleBingoCardMap {
		verticleBingoCard = append(verticleBingoCard, value)
	}

	return verticleBingoCard
